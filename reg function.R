# This is an function named 'regfn'
# After running a regression in R
...
#
#Then use the regfn function. Here dataset is the data used in regression and regression is the regression name.
#
#regfn(dataset = C_cornbelt,regression = reg1)
# Here C_cornbelt is my dataset and reg1 is the regression

#Declaimer
#I have only tries this function with plm. But it can work with other functions as well.

regfn<- function(dataset,regression){
  z<- dataset
  x<- regression
  library(tidyverse)
  library(openxlsx)
  ...
  #SE
  SE<- c(paste0("(",format(round(res$coefficients[,2],3),nsmall =3),")"))
